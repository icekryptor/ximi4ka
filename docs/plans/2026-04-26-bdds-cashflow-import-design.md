...
- `parent_id` UUID NULLABLE FK → `categories.id` — для иерархии (Сырьё → Реактивы)
...
### Точка — реальная структура (откалибровано)

Файл: `40802810220000929747 044525104 (PERIOD).xlsx`
- Лист «Операции NNNNNNNN» (сначала — основной), второй лист «Выписка NNNNNNNN» — сводка (входящий/исходящий остаток, итоги)
- Заголовки в строке 2, данные с строки 3
- 13 колонок:
  | # | Колонка | Что |
  |---|---|---|
  | 1 | Номер документа | → `external_id` |
  | 2 | Дата документа | дата (datetime) |
  | 3 | Дата операции | альтернативная дата |
  | 4 | Счёт | наш счёт |
  | 5 | Контрагент | имя |
  | 6 | ИНН контрагента | → `counterparty_inn` |
  | 7 | БИК банка контрагента | |
  | 10 | Счёт контрагента | |
  | 11 | Списание | expense (если заполнено) |
  | 12 | Зачисление | income (если заполнено) |
  | 13 | Назначение платежа | → `description` |

**Замечание:** в выписке Точки видно, что у клиента несколько собственных счетов внутри одной Точки (40802810220000929747 ↔ 40802810301500508985 «Налоги»). Это значит — трансферы между своими счетами **внутри одного банка** тоже происходят. Дедупликация должна работать не только межбанком, но и между всеми привязанными счетами клиента.

### Озон Банк — реальная структура (откалибровано)

Файл: `receipt_PERIOD.xlsx`
- Один лист «Выписка»
- Шапка с метаданными в строках 1-11 (название банка, период, ИНН клиента, итоги, остатки)
- Заголовки: основные в строке 12, под-заголовки в строке 13 (одна колонка «Контрагент» разбита на две под-колонки)
- Данные с строки 14
- Колонки:
  | # | Колонка (строка 12) | Под-колонка (строка 13) | Что |
  |---|---|---|---|
  | 2 | Дата | | дата (формат `DD.MM.YYYY`) |
  | 3 | Номер документа | | → `external_id` |
  | 4 | Дебет | | expense (если заполнено) |
  | 5 | Кредит | | income (если заполнено) |
  | 6 | Контрагент | Наименование, ИНН | многострочная ячейка `Имя\nИНН:NNNNNNNNNN` |
  | 7 | Контрагент | Счёт, БИК банка | многострочная `Р/С:...\nБИК:...` |
  | 9 | Назначение платежа | | → `description` |

**Особенности Озона:**
- ИНН не в отдельной колонке, а внутри текста ячейки — извлекаем регуляркой `/ИНН:(\d{10,12})/`
- Дата — строка `DD.MM.YYYY`, не Excel datetime; парсер парсит и нормализует к ISO
- Между шапкой и данными есть строка 11 (пустая) — парсер отступает от строки заголовков

### Общая стратегия парсера

- Авто-определение банка по сигнатурам: «ООО "ОЗОН БАНК"» в строке 1 → ozon; имя листа начинается с «Операции» → tochka
- Если автоопределение не сработало → юзер выбирает счёт вручную
- Парсер каждой выписки ищет колонки по словам в заголовке (выдержит изменение порядка), не по индексу
- На пустые/невалидные строки (промежуточные итоги, секции) — пропуск с пометкой `skipped` в счётчике

**Обновление дизайна выполнено по реальным файлам, доп. примеры от пользователя не требуются.**
...
**Иерархия категорий (subcategories):** в БДДС родитель показывается как сворачиваемая группа, дети — отдельными строками внутри. По умолчанию — свёрнуто (показывается сумма по родителю). При импорте: если правило ведёт к родительской категории — транзакция сохраняется на родителя; если правило знает дочернюю — на дочернюю. Когда парсер не уверен в субкатегории — строка идёт в превью с жёлтым флагом «требует ручной разметки субкатегории».
...
- ✅ Структура выписок Точки и Озона откалибрована по реальным файлам (см. секцию «Bank Statement Parser»)
- ✅ Субкатегории — поддерживаются через `categories.parent_id`. Если парсер не уверен в субкатегории — отправляет в превью на ручную разметку
- ✅ Стартовый остаток на счетах — 0 (юзер выгрузит весь архив транзакций с 2023, движения заполнят остатки)
