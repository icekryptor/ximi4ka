...
CREATE TABLE IF NOT EXISTS channel (
  ...
  integration_status  VARCHAR(20) NOT NULL DEFAULT 'manual' CHECK (integration_status IN ('manual', 'api_connected', 'api_planned')),
  active              BOOLEAN NOT NULL DEFAULT true,
  config_json         JSONB,
  sort_order          INT NOT NULL DEFAULT 0,
  created_at          TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS content_asset (
  ...
  storage         VARCHAR(20) NOT NULL CHECK (storage IN ('supabase', 'external')),
...
CREATE TABLE IF NOT EXISTS content_metric_snapshot (
  ...
  captured_by         VARCHAR(20) NOT NULL DEFAULT 'manual' CHECK (captured_by IN ('worker', 'manual')),
  views               INT,
  likes               INT,
  comments            INT,
  shares              INT,
